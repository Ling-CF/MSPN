...
def show_muti_img(imgs, targets, index):
    ...
    if d == 27:
        ...
        if save_img == 'y':
            ...
            save_name = './predictions/KTH/{}_pred.png'.format(name)
            cv2.imwrite(save_name, stack_img*255)
            tar_name = './predictions/KTH/{}_real.png'.format(name)
            cv2.imwrite(tar_name, stack_tar*255)



def test_model():
    model=MSPNet(channels=(64, 128, 256, 512, 1024), layers=4, in_ch=3, hidden_dim=64, BN=False)
    test_data_path='/mnt/DevFiles_NAS/Ling.cf/dataset/{}/test2'.format('Human3.6M')       # the path where test data is stored
    state_path='./models/adv_MSPN_Human3.6M_None.pt'                                      # the path where model state is stored
    PredSteps=5               # the number of time steps in the long term prediction
    show_img=True             # bool: visualize image or not
    ...
    with torch.no_grad():
        for curdir, dirs, files in os.walk(test_data_path):
            ...
            for file in files:
                ...
                test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=1, drop_last=True)
    ...
    print('mean ssim: ', '\n', mean_ssim, '\n', '0-5: ', np.mean(mean_ssim[:5]), '\n', '5-15: ', np.mean(mean_ssim[5:]))
    print("mean psnr: ", '\n', mean_psnr, '\n', '0-5: ', np.mean(mean_psnr[:5]), '\n', '5-15: ', np.mean(mean_psnr[5:]))
    print("mean lpips: ", '\n', mean_lpips, '\n', '0-5: ', np.mean(mean_lpips[:5]), '\n', '5-15: ',np.mean(mean_lpips[5:]))
